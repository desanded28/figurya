...
def upsert_figurine(conn: sqlite3.Connection, fig_data: dict) -> int:
    ...
    now = time.time()
    tags_json = json.dumps(fig_data.get("tags", []))
    ...
    """, (
        fig_data.get("name", ""),
        fig_data.get("character", ""),
        fig_data.get("series", ""),
        fig_data.get("manufacturer", ""),
        fig_data.get("price"),
        fig_data.get("currency", "JPY"),
        fig_data.get("image_url", ""),
        fig_data["product_url"],
        fig_data.get("store", ""),
        fig_data.get("availability", "unknown"),
        fig_data.get("rating"),
        tags_json,
        fig_data.get("description", ""),
        now,
    ))
    conn.commit()
    # Get the id
    row = conn.execute(
        "SELECT id FROM figurines WHERE product_url = ?", (fig_data["product_url"],)
    ).fetchone()
    return row["id"]
